...
1. 对于 nonstatic member function，指向其的指针是一个函数地址，但是依然需要类对象来进行调用，因为必须传入一个 this 指针。而 static member function 的类型就是一个函数指针。

2. 指向 virtual function 的指针：由于多态，调用哪一个 virtual function 是未知的，所以函数地址在编译期间是未知的，所能知道的仅仅是 virtual function 在其对应类中的 virtual table 中的索引值，所以此时指向 member function 的指针返回值是一个索引。
   ```cpp
   Base* p = new Derived;
   p->f();   // 调用的是 Derived::f
   ```
   ...
   **编译器在编译这行代码时：**
   ...
   - 只知道 `p` 的**静态类型**是 `Base*`
   - **不知道运行时 `p` 指向的是 `Base` 还是 `Derived`**

   所以此时成员函数指针可能指向虚函数，也可能指向正常的函数，那么指针内部就应该有策略来区分是哪种情况，因为仅仅通过返回类型和参数无法区分这个函数指针指向的是否是虚函数。

### 4.5 Inline Functions

1. 编译器处理 inline 函数有两个阶段：

   * 分析函数定义，决定是否可以成为 inline 函数，**如果不可以，会在编译模块内部声明为一个 static 函数**，并在被编译模块内生成函数定义。
   * 真正的 inline 函数扩展操作在调用点上，会带来参数求值操作以及临时对象的管理。
