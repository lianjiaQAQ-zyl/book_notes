...
1. 纯虚函数可以只声明而不进行定义，但是**对于纯虚析构函数而言，一定要提供定义**，因为每一个 derived class 的析构函数会被编译器加以扩展，以静态调用的方式调用其 “每一个 virtual base class” 以及 “上一层 base class" 的析构函数。所以如果缺少定义，链接就会失败。
...
1. 当我们定义一个类对象的时候，如果类对象有一个构造函数（不管是用户提供还是编译器合成），它会被调用，但是编译器可能会对原函数做以下扩充操作：
   * 构造函数中的**成员初始化列表**中的数据成员的初始化操作会**按照类中成员变量的声明顺序**放进构造函数本身。
   * 如果有一些数据成员没有出现在成员初始化列表，那么调用这些数据成员的默认构造函数。
   * 如果类对象有虚表指针，那么由编译器来设定正确的初始值。
   * 上一层的 Base Class 的构造函数被调用，**按照 base class 的声明顺序**。
     * 如果 base class 出现在成员初始化列表，那么根据参数调用相应的构造函数。
     * 如果 base class 没有出现在成员初始化列表但是有默认构造函数，则调用默认构造函数。
     * 如果 base class 是多重继承体系下的第二或后继的 base class，那么需要调整 this 指针。
   * 所有的虚基类构造函数被调用，从左到右，从最深到最浅。
     * 如果  class 出现在成员初始化列表，那么根据参数调用相应的构造函数。
     * 如果  class 没有出现在成员初始化列表但是有默认构造函数，则调用默认构造函数。
     * class 中的每一个 virtual base class subobject 的偏移量（offset）必须在执行期可以获取。
     * 如果 class object 为最底层的 class，它的构造函数可能被调用，所以相关机制也要放进来。
